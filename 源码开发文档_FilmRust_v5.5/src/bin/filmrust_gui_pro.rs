...
impl FilmRustPro {
    ...
    fn color_layer_params(&self) -> (f32, f32, f32) {
        for l in &self.layers.layers {
            if let LayerType::Color { warmth, tint, saturation } = l.layer_type {
                if l.visible { return (warmth, tint, saturation) }
            }
        }
        (0.0, 0.0, 1.0)
    }
    ...
    fn new(_cc: &eframe::CreationContext) -> Self {
        ...
        layers.add(Layer::new("色彩".into(), LayerType::Color { warmth: 0.0, tint: 0.0, saturation: 1.0 }));
    ...
    fn film_base_params(&self) -> (String, f32, f32) {
        match self.film_base() {
            Some(l) => match &l.layer_type { LayerType::FilmBase{stock_id,strength,grain,..}=>(stock_id.clone(),*strength,*grain), _ => (String::new(),100.0,100.0) },
            None => (String::new(),100.0,100.0)
        }
    }
    ...
    fn do_process(img: &DynamicImage, stock_id: &str, strength: f32, grain: f32, warmth: f32, _tint: f32, sat: f32) -> ProcessResult {
        let factor = strength/100.0;
        match find_filmr_stock(stock_id) {
            Ok(stock) => {
                let config = SimulationConfig {
                    exposure_time:1.0, auto_levels:true, white_balance_mode:filmr::WhiteBalanceMode::Off,
                    ...
                    warmth, saturation:sat*(1.0+(factor-1.0)*0.15), ..Default::default()
                };
    ...
    fn trigger_develop(&mut self, ctx: &egui::Context) {
        ...
        let (sid, strength, grain) = self.film_base_params();
        let (warmth, tint, sat) = self.color_layer_params();
        ...
        std::thread::spawn(move || {
            let r = Self::do_process(&img, &sid, strength, grain, warmth, tint, sat);
            let _ = tx.send(r);
        });
        ctx.request_repaint();
    }
    ...
    fn do_export_one(&mut self, path: &PathBuf, fmt: ExportFormat) -> bool {
        ...
        let (sid,s,grain) = self.film_base_params();
        let (warmth,tint,sat) = self.color_layer_params();
        let r = Self::do_process(&full, &sid, s, grain, warmth, tint, sat);
        ...
        match &mut layer.layer_type {
            LayerType::FilmBase{stock_id,strength,grain,auto_levels}=>{
                ...
                egui::ComboBox::from_id_salt("stockp").width(200.0).selected_text(&cn).show_ui(ui,|ui|{
                    for (i,p) in self.presets.iter().enumerate(){ if ui.selectable_label(false,&p.name).clicked(){*stock_id=p.id.clone();self.style_idx=i;} }
                });
    ...
    fn render_preview(&mut self, ui: &mut egui::Ui) {
        if self.original_tex.is_none() {
            ui.vertical_centered(|ui|{
                ...
                ui.add_space(12.0);
                ui.label(egui::RichText::new("涵盖 Kodak · Fujifilm · Ilford · CineStill · Polaroid · Agfa 等 57 种经典胶片风格").size(13.0).color(self.text_dim()));
                ui.add_space(4.0);
