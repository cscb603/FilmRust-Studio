...
static FILM_STYLES: &[StyleInfo] = &[
    ...
    StyleInfo {
        key: "lomography_color_chrome",
        name: "Lomography Color Chrome",
        desc: "LOMO艺术，高对比偏色",
        group: "⭐ 常用",
        is_bw: false,
        is_warm: 1,
    ...
    // 艺术增强版
    StyleInfo {
        key: "kodak_portra_400_artistic",
        name: "Kodak Portra 400 Artistic",
        desc: "艺术版Portra，增强色彩分离",
        group: "⭐ 常用",
        is_bw: false,
        is_warm: 2,
        saturation: 7,
    ...
    StyleInfo {
        key: "fujifilm_velvia_50_artistic",
        name: "Fujifilm Velvia 50 Artistic",
        desc: "增强版Velvia，极致鲜艳",
        group: "⭐ 常用",
        is_bw: false,
        is_warm: -2,
    ...
    StyleInfo {
        key: "kodak_tri_x_400_artistic",
        name: "Kodak Tri-X 400 Artistic",
        desc: "增强版，更强颗粒对比",
        group: "黑白",
        is_bw: true,
        is_warm: 0,
    ...
    StyleInfo {
        key: "ilford_hp5_plus_400_artistic",
        name: "Ilford HP5 Plus 400 Artistic",
        desc: "增强版HP5，更强颗粒",
        group: "黑白",
        is_bw: true,
        is_warm: 0,
    ...
    StyleInfo {
        key: "fujifilm_neopan_100",
        name: "Fujifilm Neopan 100",
        desc: "日系低感黑白，极致细腻",
        group: "黑白",
        is_bw: true,
        is_warm: 0,
    ...
    StyleInfo {
        key: "polaroid_spectra_color",
        name: "Polaroid Spectra Color",
        desc: "宽幅宝丽来，偏冷调",
        group: "宝丽来",
        is_bw: false,
        is_warm: 0,
    ...
    StyleInfo {
        key: "polaroid_100_color",
        name: "Polaroid 100 Color",
        desc: "老式宝丽来100，褪色怀旧",
        group: "宝丽来",
        is_bw: false,
        is_warm: 3,
    ...
    // ========== 特殊/小众 (Special) ==========
    StyleInfo {
        key: "lomography_lomochrome_purple",
    ...
    StyleInfo {
        key: "ferrania_solaris_400",
        name: "Ferrania Solaris 400",
        desc: "意式暖调，复古褪色感",
        group: "特殊/小众",
        is_bw: false,
        is_warm: 3,
...
// ============================================================
fn find_filmr_stock_by_key<'a>(
    presets: &'a [std::rc::Rc<filmr::FilmStock>],
    key: &str,
) -> Option<&'a filmr::FilmStock> {
    ...
    let clean_words: Vec<&str> = search
        .split(' ')
        .filter(|w| w.len() > 1 && !manufacturers.contains(w))
        .collect();
    let clean_nospace: String = clean_words
        .join("")
        .chars()
    ...
    presets
        ...
        .find(|s| {
            ...
            sn == search
                || sn.contains(&search)
                || search.contains(&sn)
                || sn_nospace.contains(&clean_nospace)
                || clean_nospace.contains(&sn_nospace)
...
fn find_dcraw() -> Option<PathBuf> {
    let candidates = [PathBuf::from("dcraw.exe"), PathBuf::from("dcraw")];
    for c in &candidates {
        if c.exists() {
            return Some(c.clone());
        }
...
fn convert_raw_with_dcraw(raw_path: &Path) -> anyhow::Result<image::DynamicImage> {
    let dcraw = find_dcraw().ok_or_else(|| {
        anyhow::anyhow!(
    ...
    let final_path = if dcraw_out.exists() {
        dcraw_out
    } else {
        tiff_path.clone()
    };
    ...
    let img = image::open(&final_path).with_context(|| "读取 dcraw 输出的 TIFF 失败")?;
...
// ============================================================
//  图片分析：针对全部风格智能评分推荐（与 GUI/JSX 同步）
// ============================================================
fn analyze_image(image_path: &Path) -> AnalyzeResult {
    ...
    if is_night {
        ...
        tags.push("一般室内".to_string());
    } else if is_bright {
        tags.push("明亮日拍".to_string());
    }
    if is_warm {
        tags.push("原片偏暖".to_string());
    } else if is_cool {
        tags.push("原片偏冷".to_string());
    }
    if is_flat {
        tags.push("低对比/灰".to_string());
    }
    if skin_tone_ratio > 0.08 {
        tags.push("含皮肤/人像".to_string());
    }
    ...
    for style in FILM_STYLES {
        ...
        if is_low_light {
            if style.is_night_ok {
                score += 10.0;
            }
            if style.saturation >= 5 {
                score += 5.0;
            }
        }

        if is_bright && (style.is_landscape || style.saturation >= 7) {
            score += 15.0;
        }

        if is_warm && style.is_warm < 0 {
            score += 12.0;
        } else if is_warm && style.is_warm > 0 {
            score -= 8.0;
        } else if is_cool && style.is_warm > 0 {
            score += 10.0;
        } else if is_cool && style.is_warm < 0 {
            score -= 6.0;
        }

        if skin_tone_ratio > 0.08 && style.is_portrait {
            score += 20.0;
        }
        if skin_tone_ratio > 0.15 && style.is_portrait {
            score += 10.0;
        }
        if skin_tone_ratio > 0.08 && style.is_bw {
            score -= 12.0;
        }
        if skin_tone_ratio < 0.02 && style.is_landscape {
            score += 12.0;
        }

        if is_flat && style.saturation <= 5 {
            score += 8.0;
        } else if is_flat && style.saturation >= 8 {
            score -= 8.0;
        }

        if style.is_bw && is_flat {
            score += 10.0;
        }
        if style.is_bw && !is_flat && saturation_est > 50.0 {
            score -= 10.0;
        }

        // 宝丽来：需要更自然的输入
        if style.is_instant && !is_night {
            score -= 3.0;
        }
...
fn process_image(
    ...
    let mut rgb = load_image(input_path)?.to_rgb8();
    ...
    // 后处理层默认全部关闭，用户可通过 GUI 手动启用
    // CLI 模式不自动应用肤色/色调分离/锐化
...
fn main() -> anyhow::Result<()> {
    ...
    if let Some(analyze_path) = &cli.analyze {
        ...
        let json = serde_json::to_string(&result).with_context(|| "序列化分析结果失败")?;
