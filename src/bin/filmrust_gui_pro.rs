...
impl FilmRustPro {
    ...
    fn recomposite_from_cache(&mut self) {
        // 优先使用预览缩略图缓存（1600px，滑块响应快）
        let base = if let Some(preview) = &self.preview_cache {
            preview
        } else if let Some(full) = &self.cached_filmr_base {
            full
        } else {
            return;
        };
    ...
    fn do_export_one(&mut self, path: &Path, fmt: ExportFormat) -> bool {
        ...
        if ok {
            ...
            self.status_ok = true;
            // 打开文件夹并定位到文件（同一文件夹只打开一次，避免重复弹出）
            if let Some(parent) = out.parent() {
                if self.opened_folders.insert(parent.to_path_buf()) {
                    open_folder_and_select(&out);
                }
            }
        } else {
            self.status = format!("导出失败: {}", out.display());
            self.status_ok = false;
        }
        ok
    }
...
fn rebuild_tiff_exif(data: &[u8], le: bool) -> Option<Vec<u8>> {
    ...
    for i in 0..n0 {
        ...
        if byte_size > 4 && off + byte_size <= data.len() {
...
/// PNG CRC-32（ISO 3309, 多项式 0xEDB88320，OnceLock 缓存查找表）
    ...
    fn render_layer_properties_inner(&mut self, ui: &mut egui::Ui, idx: usize) {
            ...
            LayerType::LightLeak {
                ...
                ui.horizontal(|ui| {
                    for (i, name) in positions.iter().enumerate() {
                        let btn = egui::Button::new(*name).min_size(egui::vec2(56.0, 22.0));
                        let btn = if i == *position as usize {
                            btn.fill(ui.visuals().selection.bg_fill)
                        } else {
                            btn
                        };
                        if ui.add(btn).on_hover_text(position_hints[i]).clicked() {
                            *position = i as u8;
                            dirty_post = true;
                        }
                    }
        ...
        if let Some(last_release) = self.last_slider_release {
            if last_release.elapsed().as_millis() >= 300 && self.dirty_post {
                if self.cached_filmr_base.is_some() {
                    self.recomposite_from_cache();
                    ui.ctx().request_repaint();
                }
                self.dirty_post = false;
                self.last_slider_release = None;
            } else {
                ui.ctx().request_repaint(); // 持续检查
            }
        }
    }
}
        ...
        win.show(ui.ctx(), |ui| {
            ...
            if let Some(idx) = ci {
                ...
                } = &mut self.layers.layers[idx].layer_type
                {
                    ...
                    ui.add_space(8.0);
                    ui.add(egui::Slider::new(contrast, -1.0..=1.0).text("中间调对比度"));
