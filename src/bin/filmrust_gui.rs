...
use egui::{pos2, vec2, CentralPanel, ColorImage, IconData, TextureHandle, Vec2};
...
impl FilmRustGui {
    ...
    fn load_image_for_display(&mut self, ctx: &egui::Context) {
        ...
        match image::open(path) {
            Ok(img) => {
                ...
                self.status = format!(
                    "已加载: {} ({}×{})",
                    fname, self.display_img_w, self.display_img_h
                );
                self.status_ok = true;
            }
            Err(e) => {
                self.status = format!("加载失败: {} — {}", fname, e);
    ...
    fn scale_for_display(img: &image::DynamicImage, max_px: u32) -> image::DynamicImage {
        ...
        let sc = if w > h {
            max_px as f64 / w as f64
        } else {
            max_px as f64 / h as f64
        };
        img.resize_exact(
            (w as f64 * sc).round() as u32,
            (h as f64 * sc).round() as u32,
            ...
            Ok(stock) => {
                ...
                match apply_film(&rgb, &stock, &config) {
                    Ok(mut r) => {
                        ...
                        ProcessResult {
                            ok: true,
                            image: Some(image::DynamicImage::ImageRgb8(r)),
                            error: None,
                        }
                    }
                    Err(e) => ProcessResult {
    ...
    fn process_current(&mut self, ctx: &egui::Context) {
        if self.is_processing
            || self.selected_idx >= self.files.len()
            || self.original_img.is_none()
        {
            return;
        }
        ...
        thread::spawn(move || {
            let _ = tx.send(Self::do_process(img, &style_id, s, g, w, t));
            c.request_repaint();
        });
    }
            ...
            if result.ok {
                if let Some(img) = result.image {
                    ...
                    let ci =
                        ColorImage::from_rgba_unmultiplied([w as usize, h as usize], rgba.as_raw());
                    self.processed_tex =
                        Some(ctx.load_texture("processed", ci, Default::default()));
    ...
    fn export_current(&self, path: &Path) -> Result<(), String> {
        match self.processed_img {
            Some(ref img) => img.save(path).map_err(|e| format!("保存失败: {}", e)),
            None => Err("没有已处理的图片".to_string()),
        }
    }
        ...
        while let Ok(msg) = rx.try_recv() {
            match msg {
                BatchMsg::Progress(cur, total) => {
                    me.batch_current = cur;
                    me.batch_total = total;
                    me.status = format!("批量处理: {}/{}", cur, total);
                    me.status_ok = true;
                }
                BatchMsg::Done => {
                    me.batch_running = false;
                    me.status = format!("批量完成! 共 {} 张", me.batch_total);
                    me.status_ok = true;
                    keep = false;
                }
            }
            ...
            ui.horizontal(|ui| {
                ...
                if ui.button("[打开] 文件夹").clicked() {
                    if let Some(dir) = rfd::FileDialog::new().pick_folder() {
                        ...
                        if cnt > 0 {
                            self.selected_idx = self.files.len() - cnt;
                            self.load_image_for_display(&ctx);
                        }
                    }
                }
                ...
                if ui
                    .add_enabled(self.has_processed, egui::Button::new("[保存] 导出"))
                    .clicked()
                {
                ...
                if ui
                    .add_enabled(!self.files.is_empty(), egui::Button::new("[批量] 处理"))
                    .clicked()
                {
                    if let Some(dir) = rfd::FileDialog::new().pick_folder() {
                        ...
                        self.batch_total = self.files.len();
                        ...
                        thread::spawn(move || {
                            for (i, file) in files.iter().enumerate() {
                                ...
                                if let Ok(img) = image::open(file) {
                                    ...
                                    if let Some(pi) = r.image {
                                        let _ = pi.save(dir.join(&out_name));
                                    }
                                }
                ...
                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                    if self.selected_idx < self.files.len() {
                        let nm = self.files[self.selected_idx]
                            .file_name()
                            .unwrap_or_default()
                            .to_string_lossy()
                            .to_string();
                        ui.label(format!(
                            "[{}]  {}x{}",
                            nm, self.display_img_w, self.display_img_h
                        ));
                    }
                });
            });
                    ...
                    egui::ScrollArea::vertical().show(ui, |ui| {
                        ...
                        for (i, nm) in fnames.iter().enumerate() {
                            ui.horizontal(|ui| {
                                ...
                                if ui.selectable_label(i == sel, &lb).clicked() {
                                    self.selected_idx = i;
                                    self.load_image_for_display(&ctx);
                                }
        ...
        CentralPanel::default().show_inside(ui, |ui| {
            if self.files.is_empty() {
                ui.vertical_centered(|ui| {
                    ...
                    ui.add_space(8.0);
                    ui.label(
                        egui::RichText::new("拖拽图片到窗口，或点击上方 [打开] 开始").size(16.0),
                    );
                    ui.label(
                        egui::RichText::new("支持 JPG / PNG / TIFF / BMP")
                            .size(13.0)
            ...
            let orig_tex = match self.original_tex.as_ref() {
                Some(t) => t.clone(),
                None => {
                    ui.label("加载中...");
                    return;
            ...
            let img_rect =
                egui::Rect::from_min_size(pos2(area.min.x + x_off, area.min.y + margin), img_size);
            let p = ui.painter();
            ...
            if self.has_processed {
                if let Some(ref proc_tex) = self.processed_tex {
                    ...
                    p.line_segment(
                        [pos2(split_x, img_rect.min.y), pos2(split_x, img_rect.max.y)],
                    ...
                    p.text(
                        pos2(img_rect.min.x + 8.0, img_rect.min.y + 8.0),
                    ...
                    p.text(
                        pos2(img_rect.max.x - 8.0, img_rect.min.y + 8.0),
                    ...
                    if resp.dragged_by(egui::PointerButton::Primary) {
                        if let Some(hov) = resp.hover_pos() {
                            self.split_pos =
                                ((hov.x - img_rect.min.x) / img_rect.width()).clamp(0.05, 0.95);
                        }
                    }
                    return;
                }
            ...
            p.text(
                pos2(img_rect.center().x, img_rect.max.y - 24.0),
                egui::Align2::CENTER_CENTER,
                "选择风格后点击 [处理]",
