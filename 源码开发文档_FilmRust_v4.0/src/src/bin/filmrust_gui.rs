        ...
        match find_filmr_stock(style_id) {
            Ok(stock) => {
                ...
                match apply_film(&rgb, &stock, &config) {
                    Ok(r) => {
                        let tinted = filmrust::apply_tint_to_rgb(&r, tint);
                        ProcessResult { ok: true, image: Some(image::DynamicImage::ImageRgb8(tinted)), error: None }
                    }
...
impl eframe::App for FilmRustGui {
    fn ui(&mut self, ui: &mut Ui, _frame: &mut eframe::Frame) {
        ...
        egui::Panel::bottom("controls_panel").min_size(120.0).show_inside(ui, |ui| {
            ...
            // ========== 色调倾向控制行 ==========
            ui.horizontal(|ui| {
                if self.cast_idx >= CAST_PRESETS.len() { self.cast_idx = 0; }
                ui.label("色调:");
                for (i, &(label, w, t)) in CAST_PRESETS.iter().enumerate() {
                    let is_sel = i == self.cast_idx;
                    let color = cast_color(i);
                    let txt = egui::RichText::new(label).color(egui::Color32::WHITE);
                    let btn = egui::Button::new(txt).min_size(vec2(48.0, 22.0))
                ...
                ui.separator();
                ui.add(egui::Slider::new(&mut self.warmth, -1.0..=1.0).text("色温").suffix(" "));
                ui.add(egui::Slider::new(&mut self.tint, -1.0..=1.0).text("色调").suffix(" "));
                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                    let wv = self.warmth;
                    let tv = self.tint;
