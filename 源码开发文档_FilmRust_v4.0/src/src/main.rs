...
//  图像处理：应用胶片效果
// ============================================================
fn process_image(input_path: &Path, output_path: &Path, style_name: &str, strength: i32, grain: i32, warmth: f32, tint: f32) -> anyhow::Result<()> {
    let mut rgb = load_image(input_path)?
        .to_rgb8();

    let factor = strength as f32 / 100.0;
    let grain_factor = grain as f32 / 100.0;

    let presets = filmr::presets::get_all_stocks();
    ...
    if tint.abs() > 0.005 {
        rgb = filmrust::apply_tint_to_rgb(&rgb, tint);
    }
...
fn main() -> anyhow::Result<()> {
    ...
    process_image(&input_path, &output_path, &style_name, cli.strength, cli.grain, cli.warmth, cli.tint)
