...
//  图像处理：应用胶片效果
// ============================================================
fn process_image(input_path: &Path, output_path: &Path, style_name: &str, strength: i32, grain: i32) -> anyhow::Result<()> {
    let rgb = load_image(input_path)?
        .to_rgb8();
...
fn main() -> anyhow::Result<()> {
    ...
    process_image(&input_path, &output_path, &style_name, cli.strength, cli.grain)
