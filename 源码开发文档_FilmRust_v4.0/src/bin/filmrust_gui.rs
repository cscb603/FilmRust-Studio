...
impl Default for FilmRustGui {
    fn default() -> Self {
        ...
        Self {
            ...
            proc_result_rx: None,
            style_idx: 0,
            strength: 100.0, grain: 100.0, split_pos: 0.5,
...
impl FilmRustGui {
    ...
    fn do_process(img: image::DynamicImage, style_id: &str, strength: f32, grain: f32) -> ProcessResult {
        ...
        match find_filmr_stock(style_id) {
            Ok(stock) => {
                ...
                let config = SimulationConfig {
                    ...
                    warmth: (factor - 1.0) * 0.08,
                    ..Default::default()
                };
                match apply_film(&rgb, &stock, &config) {
                    Ok(r) => ProcessResult { ok: true, image: Some(image::DynamicImage::ImageRgb8(r)), error: None },
    ...
    fn process_current(&mut self, ctx: &egui::Context) {
        ...
        let s = self.strength; let g = self.grain;
        let c = ctx.clone();
        let (tx, rx) = mpsc::channel();
        self.proc_result_rx = Some(rx);
        thread::spawn(move || { let _ = tx.send(Self::do_process(img, &style_id, s, g)); c.request_repaint(); });
    }
    ...
    fn ui(&mut self, ui: &mut Ui, _frame: &mut eframe::Frame) {
                        ...
                        thread::spawn(move || {
                            for (i, file) in files.iter().enumerate() {
                                ...
                                if let Ok(img) = image::open(file) {
                                    let r = Self::do_process(img, &sid, s, g);
        ...
        egui::Panel::bottom("controls_panel").min_size(120.0).show_inside(ui, |ui| {
            ui.horizontal(|ui| {
                ui.vertical(|ui| {
                    ui.label("胶片风格 (57种)");
                    egui::ComboBox::from_id_salt("style_select").width(280.0)
                ...
                ui.vertical(|ui| {
                    ...
                    egui::ScrollArea::vertical().show(ui, |ui| {
                        for (i, nm) in fnames.iter().enumerate() {
                            let lb = if i == sel { format!("> {}", nm) } else { nm.clone() };
